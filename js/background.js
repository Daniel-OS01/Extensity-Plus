...
(function(root) {
  ...
  function normalizeExtensions(items, state) {
    ...
    // Bolt optimization: Cache arrays into Sets and Maps to reduce lookup complexity inside the loop from O(N * M) to O(N + M).
  ...
  async function runApplyProfile(profileName) {
    ...
    // Bolt optimization: Cache desiredIds in a Set to reduce lookup complexity in the map loop from O(N * M) to O(N).
