...
(function(root) {
  ...
  function normalizeExtensions(items, state) {
    ...
    // Convert arrays to Set/Map before looping to reduce lookup time from O(N*M) to O(N+M)
    ...
    recentList.forEach(function(id, index) { recentListMap.set(id, index); });
  ...
  async function runApplyProfile(profileName) {
    ...
    // ⚡ Bolt Performance Optimization:
    // Convert array to Set for O(1) lookup inside loop
