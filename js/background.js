...
(function(root) {
  ...
  function normalizeExtensions(items, state) {
    ...
    // ⚡ Bolt: Initialize Sets and Maps outside the loop to reduce time complexity from O(N * M) to O(N + M)
    ...
    return items.slice().sort(function(left, right) {
      ...
      var recentIndex = recentMap.get(item.id);
  ...
  async function runToggleAll() {
    ...
    // ⚡ Bolt: Cache alwaysOn Set outside the loop to optimize lookups
    var alwaysOnSet = new Set(alwaysOn);
  ...
  async function runApplyProfile(profileName) {
    ...
    // ⚡ Bolt: Cache desiredIds Set outside the loop to optimize lookups
