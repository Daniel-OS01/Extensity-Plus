...
## 2025-02-12 - O(1) Pre-computations inside Arrays.map()
**Learning:** Performing multiple array `indexOf()` inclusion checks and lookups inside a heavy mapping loop (`normalizeExtensions`) creates significant O(N * M) overhead.
**Action:** Pre-compute ES6 `Set`s outside the loop to transform O(N) inclusions into O(1) `.has()` checks. Cache reused indexOf lookups locally within the loop if values are calculated against external collections.
