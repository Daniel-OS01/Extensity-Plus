...
(function(root) {
  ...
  function normalizeExtensions(items, state) {
    ...
    // Pre-compute Set/Map for O(1) lookups instead of O(N) Array.prototype.indexOf inside the iteration loop.
    // This reduces the complexity of normalization, lowering CPU usage and execution time.
    ...
    return items.slice().sort(function(left, right) {
      ...
      var recentIdx = recentMap.has(item.id) ? recentMap.get(item.id) : -1;
      ...
      return {
        ...
        lastUsed: recentIdx === -1 ? 0 : (recentList.length - recentIdx),
