...
## 2026-05-07 - Optimize normalizeExtensions array lookups
**Learning:** In the background service worker, calling `Array.prototype.indexOf` repeatedly inside a `Array.prototype.map` loop for properties like `alwaysOn` or `favorites` causes an (N 	imes M)$ performance bottleneck for extensions with many configured profiles.
**Action:** Use ES6 `Set` and `Map` to cache lookup values outside of the loop so each item check is an (1)$ operation, reducing time complexity to (N + M)$.
