...
## 2024-05-25 - Optimize Array Lookups in Map Iterations
**Learning:** O(N) array lookups (like `indexOf`) inside an iteration loop cause significant performance overhead.
**Action:** Replace O(N) array lookups with O(1) `Set` or `Map` lookups by pre-computing them outside the loop.
