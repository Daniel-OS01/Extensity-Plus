...
## 2024-07-12 - Background Array IndexOf Iteration Optimization
**Learning:** Calling `Array.prototype.indexOf()` repeatedly inside `.map()` or `.filter()` loops during extension normalization (e.g., matching `alwaysOn`, `favorites`, `recentList`) results in $O(N^2)$ time complexity which scales poorly with large arrays and large numbers of extensions.
**Action:** When iterating over arrays and checking membership in other arrays, always pre-compute standard array checks into `Set` structures and indexed lookups into `Map` structures before the loop to reduce lookups to $O(1)$.
