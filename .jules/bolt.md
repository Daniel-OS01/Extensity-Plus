...
## 2024-05-24 - Pre-computing Sets for Array Checks Inside Map

**Learning:** Using `Array.prototype.indexOf()` multiple times inside an array mapping iteration creates an O(N*M) time complexity bottleneck. By pre-computing `Set`s for array items prior to the loop and mapping over the resulting `Set` objects, the check operates at O(1) time complexity. Using `Map` to retain an array's positional context inside loops can be equally effective.
**Action:** When working with iterating over arrays containing items that need cross-referencing against other datasets, evaluate replacing inline `.indexOf()` lookups with an initial `Set` and `Map` derivation, to reduce big-O bounds down from quadratic bounds to linear constraints.
