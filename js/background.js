...
(function(root) {
  ...
  function normalizeExtensions(items, state) {
    ...
    // Performance optimization: Pre-compute Set and Map objects for O(1) lookups
    // inside the iteration block below, replacing O(N) Array.prototype.indexOf calls.
    ...
    recentList.forEach(function(id, index) {
      if (!recentMap.has(id)) {
        recentMap.set(id, recentList.length - index);
      }
    });
