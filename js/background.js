...
(function(root) {
  ...
  function normalizeExtensions(items, state) {
    ...
    // ⚡ Bolt: Pre-compute Sets and Map to replace O(N) indexOf lookups with O(1) checks in the iteration loop below.
    ...
    return items.slice().sort(function(left, right) {
      ...
      var recentIndex = recentListMap.get(item.id);
      ...
      return {
        ...
        lastUsed: recentIndex === undefined ? 0 : (recentList.length - recentIndex),
  ...
  async function runToggleAll() {
    ...
    // ⚡ Bolt: Pre-compute Set to replace O(N) indexOf lookups with O(1) checks during toggle operation
    var alwaysOnSet = new Set(alwaysOn);
  ...
  async function runApplyProfile(profileName) {
    ...
    // ⚡ Bolt: Pre-compute Set to replace O(N) indexOf lookups with O(1) checks during profile application
