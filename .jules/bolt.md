...
## 2026-05-29 - Optimize Array Lookups in Loops
**Learning:** Using `array.indexOf` or `array.includes` inside a `.map` or loop over a large array scales poorly, essentially resulting in $O(N \times M)$ complexity.
**Action:** Replace repeated `array.indexOf` or `array.includes` lookups inside a loop with an ES6 `Set` (for presence) or `Map` (for index caching) initialized outside the loop to reduce time complexity to $O(N + M)$.
