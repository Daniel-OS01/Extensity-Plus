...
# Gemini CLI reads GEMINI.md files hierarchically — project root, then subdirectories.
# Subdirectory GEMINI.md files add to, or override, the root file for their scope.
# Use /memory show or /memory list to inspect which context files are currently active.
# Do NOT use GEMINI_SYSTEM_MD unless you intentionally want to replace the built-in
# system prompt entirely — that is an advanced override with significant consequences.

---

## Gemini CLI-Specific Notes

- Keep this file concise and project-focused. Gemini CLI loads it as context.
- For component-specific rules, place a GEMINI.md close to the relevant directory.
- Use /init to generate a starter file for a new project, then review it.
- If Gemini's behavior seems inconsistent with these rules, run /memory list to check
  which instruction files are active and whether they are conflicting.
- Prefer this file for all project coding rules. Do not split important rules across
  memory commands and this file — this file is the authoritative source.
...
4. Training data — least reliable; treat as hypothesis, not confirmed fact
...
- Exception: local imports inside a function are allowed for circular dependency avoidance,
  optional dependencies, or startup cost reduction. Add a comment explaining why.
...
- Avoid Any (Python) and any (TypeScript) — they disable type checking and hide bugs.
...
- Never retry permanent errors:
    HTTP 400, 401, 403, 404 — retrying these is wasteful and can cause real damage
...
- Use structured logs with stable event names and structured fields.
  ...
  Bad (dynamic value in message — hard to search and aggregate):
    logger.warning(f"Failed for user {user_id}")

  Good (stable message, dynamic values as structured fields):
...
- Do not add a dependency if the standard library or existing project tooling is sufficient.
...
- Docstrings document the public API contract: what, parameters, return type, errors raised.
...
- Use separate docs files for: architecture, setup instructions, runbooks, and cross-cutting
  concerns that cannot be understood from a single file.
...
## 14. How Gemini Should Communicate
