...
(function(root) {
  ...
  function normalizeExtensions(items, state) {
    ...
    // ⚡ Bolt Performance Optimization: Use Sets/Maps for O(1) lookups instead of repeated O(N) array.indexOf calls
    ...
    var recentListLength = recentList.length;
    for (var i = 0; i < recentListLength; i++) {
    ...
    return items.slice().sort(function(left, right) {
      ...
      return {
        ...
        lastUsed: recentListMap.has(item.id) ? (recentListLength - recentListMap.get(item.id)) : 0,
  ...
  async function runToggleAll() {
    ...
    // ⚡ Bolt Performance Optimization: Use Sets for O(1) lookups instead of repeated O(N) array.indexOf calls
    var alwaysOnSet = new Set(alwaysOn);
