...
(function(root) {
  ...
  function normalizeExtensions(items, state) {
    ...
    // Performance optimization: Pre-compute Sets and Maps for O(1) lookups inside the loop.
    // Replaces repeated array.indexOf() calls, reducing time complexity from O(N*M) to O(N+M).
    ...
    return items.slice().sort(function(left, right) {
      ...
      var recentIndex = recentMap.has(item.id) ? recentMap.get(item.id) : -1;
