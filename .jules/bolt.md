...
## 2024-05-24 - Pre-compute Array Iteration Lookups
**Learning:** Replaced O(N) `Array.prototype.indexOf()` lookups inside iteration loops with O(1) `Set.prototype.has()` or `Map.prototype.get()` checks to reduce time complexity to O(M + N).
**Action:** When working on array transformations inside loops, evaluate if lookups against other arrays can be optimized by pre-computing `Set`s or `Map`s before the loop.
