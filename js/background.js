...
(function(root) {
  ...
  function normalizeExtensions(items, state) {
    ...
    // ⚡ Bolt Optimization: Pre-compute sets for O(1) lookups in the items map loop
    // Expected impact: Speeds up normalization of large extension lists significantly
    var alwaysOnSet = new Set(alwaysOn);
    var favoritesSet = new Set(favorites);
    var toolbarPinsSet = new Set(toolbarPins);

    // ⚡ Bolt Optimization: Pre-compute recentList scores for O(1) map lookups instead of repeated O(n) indexOf()
    var recentMap = new Map();
    var recentLength = recentList.length;
    for (var i = 0; i < recentLength; i++) {
      if (!recentMap.has(recentList[i])) {
        recentMap.set(recentList[i], recentLength - i);
      }
    }
