...
(function(root) {
  ...
  function normalizeExtensions(items, state) {
    ...
    // Pre-compute Sets and Map to turn O(N * M) repeated array lookups into O(N + M) constant time checks
    ...
    recentList.forEach(function(id, index) { recentMap.set(id, index); });
    ...
    return items.slice().sort(function(left, right) {
      ...
      return {
        ...
        lastUsed: !recentMap.has(item.id) ? 0 : (recentList.length - recentMap.get(item.id)),
