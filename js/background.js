...
(function(root) {
  ...
  function normalizeExtensions(items, state) {
    ...
    // Performance optimization: Pre-compute Sets from arrays to replace O(N) `indexOf`
    // lookups inside the iterator with O(1) `Set.has` checks.
    ...
    return items.slice().sort(function(left, right) {
      ...
      // Performance optimization: Cache the result of expensive indexOf operation
      var recentListIndex = recentList.indexOf(item.id);
      ...
      return {
        ...
        lastUsed: recentListIndex === -1 ? 0 : (recentList.length - recentListIndex),
  ...
  async function runToggleAll() {
    ...
    // Performance optimization: Pre-compute Set to replace O(N) `indexOf`
    // lookups inside the iterator with O(1) `Set.has` checks.
    var alwaysOnSet = new Set(alwaysOn);
